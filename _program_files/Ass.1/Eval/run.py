...
def main():
    ...
    try:
        # Sending total runs
        client.Put(Message(key=Key(type="world", name="director-run", namespace="sbu-ai-2021-snake"), content=json.dumps({"total": len(config['runs'])})))
        
        for index, run in enumerate(config["runs"]):
            print(f"RUN {index + 1}")
            
            client.Put(Message(key=Key(type="world", name="director-run", namespace="sbu-ai-2021-snake"), content=str(index)))
            ...
            sleep(.25)
            client.Put(Message(key=Key(type="world", name="director-init", namespace="sbu-ai-2021-snake"), content=json.dumps(message)))

            # Waiting for agent to receive init data and be ready
            for action in client.Watch(Key(type="world", name="actor-init", namespace="sbu-ai-2021-snake")):
            ...
            message = world.perceive(agent)
            message['status'] = "success"
            sleep(.25)
            client.Put(Message(key=Key(type="action", name="director", namespace="sbu-ai-2021-snake"), content=json.dumps(message)))
            print("Sent kickstart")
            # gui = cli_ui()

            startTime = time()

            for action in client.Watch(Key(type="action", name="actor", namespace="sbu-ai-2021-snake")):
                now = time()
                # if now - startTime > 600:
                #     # Agent had exception
                #     print("Agent reached the timeout")
                #     client.Put(Message(key=Key(type="action", name="director", namespace="sbu-ai-2021-snake"), content=json.dumps({"status": "timed_out"})))
                ...
                if response['action'] == "fail":
                    ...
                    client.Put(Message(key=Key(type="action", name="director", namespace="sbu-ai-2021-snake"), content=json.dumps({"status": "failed"})))
                ...
                status = world.take_action(response['action'], agent)
                newState = world.perceive(agent)
                # gui.display(world.state)
                
                if world.goal_test():
                    # Agent solved the problem
                    client.Put(Message(key=Key(type="action", name="director", namespace="sbu-ai-2021-snake"), content=json.dumps({"status": "victory"})))
                    results.append({
                        "index": index,
                        # "run": run,
        ...
        print(tb, e)
        client.Put(Message(key=Key(type="action", name="director", namespace="sbu-ai-2021-snake"), content=json.dumps({"status": "exception", "traceback": tb, "exception": str(e)})))
